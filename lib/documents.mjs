...
import { isPublishedSlug, relocateRetiredLinks, formatRegulations } from './regulation-format.mjs';
...
export function splitMatter(source, sourcePath = 'document') {
  ...
  if (data.effective_date != null) {
    ...
    if (typeof date !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(date) || Number.isNaN(Date.parse(`${date}T00:00:00Z`)) || new Date(`${date}T00:00:00Z`).toISOString().slice(0,10) !== date) throw new Error(`${sourcePath}: invalid effective_date.`);
  }
...
export function renderMarkdown(markdown) {
  ...
  html = formatRegulations(relocateRetiredLinks(html));
...
export function getDocuments(directory = CONTENT_DIR) {
  const documents = walk(directory).filter(filename => isPublishedSlug(filename.slice(0,-3))).map(filename => {
