...
test('article-card formatting retains wording, heading anchors and table contents',()=>{
 const source='<h2 id="chapter">제1장 총칙</h2><h3 id="a1">제1조 목적</h3><p>원문 3,000원 (예외)</p><ol><li>첫 항</li></ol><h3 id="a2">제2조 한도</h3><table><tbody><tr><td>10,000원</td></tr></tbody></table><h2>부칙</h2><p>2026년 9월 1일</p>';
 ...
 assert.match(formatRegulations('<h3>제1조(명칭)</h3><p>본회(이하)</p>'), /class="art-title">명칭<\/span>/);
 assert.match(formatRegulations('<h3>제1조(명칭)</h3><p>본회(이하)</p>'), /본회\(이하\)/);
});
...
test('user-specified amounts and operational access facts are not lost',()=>{
 ...
 assert.match(getDocument(['clubroom']).body,/운영진의 사전 승인/);
 const cctv=getDocument(['cctv']).body;
 for(const value of ['윤민기','rcyindcu@gmail.com','동아리방 내부 1개','Xiaomi Home','1년','동아리 임원','영상과 함께 음성 수집'])assert.ok(cctv.includes(value),value);
 assert.match(getDocument(['media']).body,/가입 시 안내한/);
});
