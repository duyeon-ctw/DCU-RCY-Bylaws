...
test('article labels are readable and the bylaw name contains no literal stars', () => {
  ...
  for (let n = 1; n <= 43; n++) {
    const heading = new RegExp('^### 제' + n + '조 ', 'm');
    if (n >= 14 && n <= 18) assert.doesNotMatch(bylaws.body, heading);
    else assert.match(bylaws.body, heading);
  }
...
test('remaining chapters and their original article order are preserved in the outline', () => {
  const doc = getDocument(['bylaws']);
  const grouped = groupOutline(doc.toc);
  assert.equal(grouped.filter(group => /^제\d+장/.test(group.title)).length, 8);
  assert.ok(!grouped.some(group => group.title === '제4장 총회'));
