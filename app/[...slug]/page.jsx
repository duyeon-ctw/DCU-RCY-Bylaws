...
export async function generateMetadata({ params }) {
  ...
  return doc
    ? { title: doc.title, description: doc.description, alternates: { canonical: doc.href } }
    : {};
}
    ...
    <>
      ...
      <header className="document-header">
        ...
        <div className="metadata">
          ...
          {doc.effective_date && (
            <span>
              {doc.source_type === 'supplement' ? '지정 시행일' : '시행일'}{' '}
              <time dateTime={doc.effective_date}>{formatDate(doc.effective_date)}</time>
            </span>
          )}
          <span className="status">{doc.status}</span>
          {doc.updated_date && (
            <span>개정 작성일 {formatDate(doc.updated_date)}</span>
          )}
        </div>
      </header>
      {doc.source_type === 'supplement' && (
        <div className="notice">
          이 문서는 보충 작성안입니다. 미기재 사항과 적용 보류 항목은 확인·고지 절차를 거치기 전 적용하지 않습니다.
        </div>
      )}
      <details className="page-outline">
        <summary>이 문서 차례</summary>
        <nav aria-label="현재 문서 목차">
          {doc.toc.map((entry) => (
            <a
              key={entry.id}
              href={`#${entry.id}`}
              className={entry.level >= 3 ? 'subheading' : undefined}
            >
              {entry.title}
            </a>
          ))}
      ...
      <div className="document-actions">
        <a href={`/downloads/${doc.slug}.md`} download>Markdown 원본</a>
      </div>
    </>
  );
}
